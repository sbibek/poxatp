...
class Tutorial (object):
  ...
  def craft_packet(self, packet_in, event):
    p = event.parse()
    print("{}-".format(p.__class__.__name__))
    while p.next is not None:
      print "{}".format(p.next.__class__.__name__)
      p = p.next

  def resend_packet (self, packet_in, out_port, event):
    """
    ...
    msg.data = packet_in
    self.craft_packet(packet_in, event)
  ...
  def act_like_switch (self, packet, packet_in, event):
    ...
    if packet.dst in self.mac_to_port:
      ...
      self.resend_packet(packet_in, of.OFPP_ALL, event)
